/**
 * Clase que implementa un recomendador siguiendo el método de filtrado colaborativo.
 */
class Recomendador {
    /**
    ...
    correlacion_pearson(usuario_u, usuario_v) {
        ...
        let similitud = sumatorio1 / (Math.sqrt(sumatorio2) * Math.sqrt(sumatorio3));
        if (similitud < 0) {
            similitud = (similitud - (-1)) / (1 - (-1));
        }
        return similitud;
    }
